...
from basicsr.models.archs.FPM_arch import FPM
from skimage import img_as_ubyte
from pdb import set_trace as stx

parser = argparse.ArgumentParser(description='Image Dehazning using FPM')

parser.add_argument('--input_dir', default='./data/test/hazy', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./Result', type=str, help='Directory for results')
parser.add_argument('--weights', default='./data/models/net_g_48000.pth', type=str, help='Path to weights')
...
####### Load yaml #######
yaml_file = './Dehaze/Options/RealDehazing_FPM.yml'
import yaml
...
model_restoration = FPM(**x['network_g'])
...
for dataset in datasets:
    ...
    files = natsorted(glob(os.path.join(inp_dir, '*.png')) + glob(os.path.join(inp_dir, '*.png')))
    with torch.no_grad():
